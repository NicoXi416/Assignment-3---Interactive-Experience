"""
VOID RUNNER — a hardcore bullet-hell shooter in pure Python (turtle)
...
Design notes:
- Single draw turtle for all sprites for performance.
- Separate background-layer turtle for static starfield.
- Game state updated ~30 FPS via ontimer.
- Difficulty scales over time: spawn rate, enemy speed, patterns.
- Power-ups: damage/ROF upgrades; Bomb clears danger; Extra life.
- No external dependencies, cross-platform with standard Python.
...
screen.title("VOID RUNNER — Bullet Hell (turtle)")
screen.tracer(0, 0)
...
class Game:
    ...
    def reset(self):
        ...
        self.key = {"up":False, "down":False, "left":False, "right":False,
                    "fire":False}
        self.player = {"x":0.0, "y":-HEIGHT/2+80, "vx":0.0, "vy":0.0,
                       "lives":3, "bombs":2, "invuln":0, "power":1,
                       "rof_cd":0}
        self.bullets = []         # player bullets
        self.enemies = []         # enemies
        self.ebullets = []        # enemy bullets
        self.powerups = []        # {'x','y','type'}
        self.particles = []       # tiny explosion dots
        self.spawn_cd = 30
        self.help = True
        pen.clear(); ui.clear()
        self.draw_starfield()
...
def dist2(a, b): dx=a["x"]-b["x"]; dy=a["y"]-b["y"]; return dx*dx+dy*dy
...
# -------------------- Spawning --------------------
def spawn_enemy():
    # Patterns with escalating difficulty
...
def do_bomb():
    ...
    G.player["bombs"] -= 1
    # Clear nearby bullets + damage enemies in radius
...
def update_bullets():
    ...
    for b in G.bullets:
        b["x"] += b["vx"]; b["y"] += b["vy"]
        if within_bounds(b["x"], b["y"], pad=40):
            nb.append(b)
    G.bullets = nb
...
def update_enemies():
    for e in G.enemies[:]:
        ...
        if e["cd"] <= 0 and len(G.ebullets) < MAX_ENEMY_BULLETS:
            e["cd"] = int(40 - min(28, G.difficulty*3)) + random.randint(-6, 6)
            shoot_pattern(e)
...
def shoot_pattern(e):
    ...
    if mode == "turret":
        # radial burst
        n = 6 + int(G.difficulty*0.8)
        for i in range(n):
            a = 2*math.pi*i/n
            vx, vy = math.cos(a)*(2.5+0.2*G.difficulty), math.sin(a)*(2.5+0.2*G.difficulty)
        ...
        for da in (-0.18, 0.0, 0.18):
            vx, vy = math.cos(ang+da)*(3.2+0.25*G.difficulty), math.sin(ang+da)*(3.2+0.25*G.difficulty)
            G.ebullets.append({"x":ex, "y":ey, "vx":vx, "vy":vy})
    else:
        # chaser: slow spiral
        base = G.frame*0.08
        for i in range(2):
            a = base + i*math.pi
            vx, vy = math.cos(a)*(2.2+0.15*G.difficulty), math.sin(a)*(2.2+0.15*G.difficulty)
            G.ebullets.append({"x":ex, "y":ey, "vx":vx, "vy":vy})
...
def update_enemy_bullets():
    ...
    for b in G.ebullets:
        b["x"] += b["vx"]; b["y"] += b["vy"]
        if within_bounds(b["x"], b["y"], pad=60):
            nb.append(b)
    G.ebullets = nb
...
def handle_collisions():
    ...
    # enemy bullets or enemies hit player
    p = G.player
...
def step_difficulty():
    ...
    if G.frame % 60 == 0:
        ...
        G.difficulty = 1.0 + elapsed / 25.0  # ramps steadily
        G.level = 1 + int(elapsed // 20)

    # spawn enemies
    G.spawn_cd -= 1
    min_cd = max(8, 26 - int(G.difficulty*3))
...
def draw_ui():
    ...
    ui.write(f"Score: {G.score}   Hi: {G.hiscore}   Lives: {G.player['lives']}   Bombs: {G.player['bombs']}   Power: {G.player['power']}   Lv: {G.level}",
